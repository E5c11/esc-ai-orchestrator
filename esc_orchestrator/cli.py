...
def main(argv=None):
    ...
    parser.add_argument("--db",type=Path,default=Path(".orchestrator/orchestrator.db")); parser.add_argument("--output",type=Path,default=Path(".orchestrator/runs"))
    ...
    args=parser.parse_args(argv); store=Store(args.db); scheduler=Scheduler(store,OpenCodeRuntime(args.opencode,args.registry),args.output); httpd=server(scheduler,store,args.host,args.port)
