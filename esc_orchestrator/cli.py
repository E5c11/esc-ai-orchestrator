...
def main(argv=None):
    ...
    args=parser.parse_args(argv); store=Store(args.db); scheduler=Scheduler(store,OpenCodeRuntime(args.opencode,args.registry),args.registry); httpd=server(scheduler,store,args.host,args.port)
